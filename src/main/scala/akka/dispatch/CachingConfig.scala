...
private[akka] class CachingConfig(_config: Config) extends Config {
  ...
  // DOTTY -> bad explicit java lang enum type
  override def getEnumList[T <: java.lang.Enum[T]](enumClass: Class[T], path: String): util.List[T] =
    config.getEnumList(enumClass, path)

  override def getEnum[T <: java.lang.Enum[T]](enumClass: Class[T], path: String): T =
  config.getEnum(enumClass, path)
