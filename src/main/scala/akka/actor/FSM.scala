...
object FSM {
  ...
  private[akka] final case class Timer(name: String, msg: Any, mode: TimerMode, generation: Int, owner: AnyRef)(
    ...
    // DOTTY => explicit return type
    private implicit val executionContext: scala.concurrent.ExecutionContext = context.dispatcher
  ...
  case class State[S, D](
    ...
    // DOTTY -> bug to be reported, removing defaults ...
    // def copy(
    //     stateName: S,
    //     stateData: D,
    //     timeout: Option[FiniteDuration],
    ...
    def forMax(timeout: Duration): State[S, D] = timeout match {
      // DOTTY => bug in copy to be reported
      case f: FiniteDuration => copy(
      ...
      case Duration.Inf      => copy(
          stateName = this.stateName,
          stateData = this.stateData,
          timeout = SomeMaxFiniteDuration,
          stopReason = this.stopReason,
          replies = this.replies
