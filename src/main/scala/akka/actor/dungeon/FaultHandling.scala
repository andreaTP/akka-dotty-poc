...
private[akka] trait FaultHandling { this: ActorCell =>
  ...
  protected def terminate(): Unit = {
    ...
    if (systemImpl.aborting) {
      ...
      // DOTTY -> bug? incompatibility with scalac probably
      children.foreach { x => x match {
          case ref: ActorRefScope if !ref.isLocal => self.sendSystemMessage(DeathWatchNotification(x, true, false))
          case _                                  =>
        }
      }
      // children.foreach {
