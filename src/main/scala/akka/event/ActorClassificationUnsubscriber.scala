...
protected[akka] class ActorClassificationUnsubscriber(bus: ManagedActorClassification, debug: Boolean)
  ...
  def receive = {
    ...
    // DOTTY -> here the compiler gets really confused...
    ...
    //   bus.asInstanceOf[akka.event.ManagedActorClassification].unsubscribe(actor)
    //   bus.unsubscribe(actor)

    case Terminated(_actor) =>
      if (debug)
        context.system.eventStream.publish(
          Logging.Debug(simpleName(getClass), getClass, s"actor $_actor has terminated, unsubscribing it from $bus"))
      // the `unsubscribe` will trigger another `Unregister(actor, _)` message to this unsubscriber;
      // but since that actor is terminated, there cannot be any harm in processing an Unregister for it.
      bus.asInstanceOf[akka.event.ManagedActorClassification].unsubscribe(_actor.asInstanceOf)
      // bus.unsubscribe(_actor)
  }
