  ...
  val versions: Map[String, Version] = {
    ...
    try {
      ...
      case ioe: IOException =>
        // DOTTY -> simplify the resolution of LogSource from the string instead of from the type
        Logging(system, getClass.getCanonicalName).warning("Could not read manifest information. {}", ioe)
    }
    manifests
  }
  ...
  def checkSameVersion(productName: String, dependencies: immutable.Seq[String], logWarning: Boolean): Boolean = {
    ...
    if (values.size > 1) {
      if (logWarning) {
        ...
        // DOTTY -> simplify the resolution of LogSource from the string instead of from the type
        Logging(system, getClass.getCanonicalName).warning(
